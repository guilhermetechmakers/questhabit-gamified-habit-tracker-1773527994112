-- QuestHabit Stripe payments schema. Idempotent.
-- Adds Stripe-related columns and tables for subscriptions, invoices, payment methods.
...
-- INSERT/UPDATE/DELETE: no policy for app users; Edge Functions use service role to reconcile webhooks

-- invoices (local copy for listing and PDF download)
...
-- payment_methods (card last4, brand, etc. — no raw card data)
...
INSERT INTO public.plans (id, stripe_price_id, name, amount_cents, currency, interval)
VALUES
  ('quest_monthly', 'price_quest_monthly_placeholder', 'Quest Monthly', 999, 'usd', 'monthly'),
  ('quest_yearly', 'price_quest_yearly_placeholder', 'Quest Yearly', 9990, 'usd', 'yearly')
