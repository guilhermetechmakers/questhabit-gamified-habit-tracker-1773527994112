/**
 * QuestHabit Stripe: attach a payment method to the customer.
 * Called after client collects card via Stripe Elements and obtains paymentMethodId.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    if (!customerId) {
      return new Response(JSON.stringify({ error: 'No Stripe customer. Complete checkout first.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle()
    if (!existing) {
      const isFirst = await supabase.from('payment_methods').select('id', { count: 'exact', head: true }).eq('user_id', user.id)
      const isDefault = (isFirst?.count ?? 0) === 0
